...
SELECT location, MAX(cast (total_deaths as int)) AS TotalDeathCount,  ROUND(MAX(( total_deaths/population))*100,5) AS PercentageDeaths
FROM PortfolioProject..CovidDeaths
WHERE continent IS NOT NULL 
--WHERE location LIKE '%india%'
GROUP BY location
...
--GLOBAL NUMBERS
SELECT  date, SUM(new_cases) AS SumNewCases, SUM(CAST( new_deaths as int) )  AS SumNewDeaths, ROUND(SUM(CAST(new_deaths as int))/SUM(new_cases)*100,5) as Percentage
FROM PortfolioProject..CovidDeaths
WHERE continent IS NOT NULL
GROUP BY date
ORDER BY 1,2
